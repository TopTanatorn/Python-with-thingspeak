...
class Blockchain:
    def __init__(self):# create the frist block with the initial value
    ...
    def mine_block(self, data: str) -> dict: #mine block method for crete the block
    ...
    def _create_block( self, data: str, proof: int, previous_hash: str, index: int) -> dict:# method for create block
    ...
    def get_previous_block(self) -> dict: # for recall the previod block
        
        return self.chain[-1]

    def _to_digest(self, new_proof: int, previous_proof: int, index: int, data: str) -> bytes:#coverst to digest
        
        to_digest = str(new_proof ** 2 - previous_proof ** 2 + index) + data
        # It returns an utf-8 encoded version of the string
        return to_digest.encode()

    def _proof_of_work(self, previous_proof: str, index: int, data: str) -> int: # PoW method of blockchain
    ...
    def _hash(self, block: dict) -> str:# hash funtion

        encoded_block = _json.dumps(block, sort_keys=True).encode()

        return _hashlib.sha256(encoded_block).hexdigest()

    def is_chain_valid(self) -> bool: # check the validation of the chain
